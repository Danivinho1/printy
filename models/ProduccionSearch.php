...
class ProduccionSearch extends Produccion
{
    ...
    public static function getEstadisticasResumen()
{
    $db = \Yii::$app->db;
    
    // Total de letreros
    $totalLetreros = $db->createCommand('SELECT COUNT(*) FROM produccion')->queryScalar();
    
    // Letreros producidos a tiempo (listos y entregados antes o en la fecha límite)
    $letrerosATiempo = $db->createCommand('
        SELECT COUNT(*) 
        FROM produccion p
        LEFT JOIN ventas v ON p.venta_id = v.id
        LEFT JOIN catalogos c ON p.empaquetado_id = c.id
        WHERE c.nombre = :nombre_listo 
        AND v.fecha_entrega >= CURDATE()
    ', [':nombre_listo' => 'Listo'])->queryScalar();
    ...
    $productosPorTipo = $db->createCommand('
        ...
        ORDER BY total DESC
    ', [':tipo_letrero' => 'tipo_letrero'])->queryAll();
    
    // Pendientes (no listos)
    $pendientes = $db->createCommand('
        SELECT COUNT(*) 
        FROM produccion p
        LEFT JOIN catalogos c ON p.empaquetado_id = c.id
        WHERE c.nombre != :nombre_listo OR c.nombre IS NULL
    ', [':nombre_listo' => 'Listo'])->queryScalar();
    
    // Con retraso (pendientes y fecha vencida)
    ...
    return [
        'total_letreros' => (int)$totalLetreros,
        'letreros_a_tiempo' => (int)$letrerosATiempo,
        'productos_por_tipo' => $productosPorTipo ?: [],
        'pendientes' => (int)$pendientes,
        'con_retraso' => (int)$conRetraso,
        'listos' => (int)$totalLetreros - (int)$pendientes
    ];
}
