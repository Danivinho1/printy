...
/* @var $searchModel app\models\PermisoSearch|null */

$this->title = 'Permisos';
$this->params['breadcrumbs'][] = $this->title;

// Conteos
$totalRegistros = (int) Permiso::find()->count();
$activosCount   = (int) Permiso::find()->where(['activo' => 1])->count();
$inactivosCount = (int) Permiso::find()->where(['activo' => 0])->count();

$formName = isset($searchModel) ? $searchModel->formName() : 'PermisoSearch';
...
<div class="permiso-index">
  ...
  <div class="filtros-diseño mb-4">
    <div class="d-flex align-items-center gap-2 mb-3">
      ...
      <a href="<?= Url::to(['permiso/index', $formName => ['activo' => 1]]) ?>"
         class="btn-filtro <?= ($filtroActivo !== null && (string)$filtroActivo === '1') ? 'active' : '' ?>">
        Activos (<?= $activosCount ?>)
      </a>

      <a href="<?= Url::to(['permiso/index', $formName => ['activo' => 0]]) ?>"
         class="btn-filtro <?= ($filtroActivo !== null && (string)$filtroActivo === '0') ? 'active' : '' ?>">
        Inactivos (<?= $inactivosCount ?>)
      </a>

      <div class="ms-auto">
        <?= Html::a('<i class="fas fa-plus me-1"></i> Nuevo permiso', ['create'], ['class' => 'btn-filtro']) ?>
      </div>
    </div>
  </div>
          ...
          [
              ...
              'buttons' => [
                  ...
                  'delete' => fn($url) => Html::a('Eliminar', $url, [
                      ...
                      'data' => ['confirm' => '¿Eliminar este permiso?', 'method' => 'post'],
                  ]),
              ],
              'header' => 'Acciones',
