    ...
    private function desactivarTogglesPorArea($produccion, $areaError)
    {
        ...
        switch ($areaError) {
            case 'impresion':
                ...
                if ($produccion->hasAttribute('diseno_impresion'))
                    $produccion->diseno_impresion = 0;
                if ($produccion->hasAttribute('corte_listo'))
                    $produccion->corte_listo = 0;
                if ($produccion->hasAttribute('fabricacion_listo'))
                    $produccion->fabricacion_listo = 0;
                break;
