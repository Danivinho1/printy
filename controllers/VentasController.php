    ...
    public function actionIndex()
{
    ...
    $dias = (int)Yii::$app->request->get('dias', 3);
    ...
    // Filtros por atención
    if ($f === 'pago') {
        $query->andWhere(['>', 'restante', 0]);
    } elseif ($f === 'proxima') {
        $query->andWhere(['between', new \yii\db\Expression('DATE(fecha_entrega)'), $hoy, $hasta]);
    } elseif ($f === 'atrasada') {
        $query->andWhere(['not', ['fecha_entrega' => null]])
              ->andWhere(['<', new \yii\db\Expression('DATE(fecha_entrega)'), $hoy]);
    }
